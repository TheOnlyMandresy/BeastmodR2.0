...
namespace Systeme\HTML\Forum\Dependencies;
...
class Comments {
    ...
    public function __construct ($idTopic, $page = 1)
    {
        $this->idTopic = $idTopic;
        $this->total = count(Table::getComments($idTopic));
        $this->page = $page;
        $comments = Table::getComments($idTopic, $this->limitation());
    ...
    {
        $datas = Table::getResponses($this->idTopic, $id);
    ...
    private function loadPagination ($pages)
    {
        ...
        if ($pages > 1) {
            ...
            if ($this->page > 1) {
                $html .= '<a href="/forum/' .$this->idTopic. '/' .ceil($this->page - ($this->page - 1)). '" class="first button-base"><img src="/img/2394__-2MnE.png" /></a>';
                $html .= '<a href="/forum/' .$this->idTopic. '/' .ceil($this->page - 1). '" class="next button-base"><img src="/img/2394__-2Mn.png" /></a>';
            } else {
            ...
            if ($this->page < $pages) {
                $html .= '<a href="/forum/' .$this->idTopic. '/' .ceil($this->page + 1). '" class="prev button-base"><img src="/img/2394__-2Mn.png" /></a>';
                $html .= '<a href="/forum/' .$this->idTopic. '/' .$pages. '" class="last button-base"><img src="/img/2394__-2MnE.png" /></a>';
            } else {
                ...
                if (($this->page + 12) > $pages) {
                    for ($i = $pages - 11; $i <= $pages; $i++) {
                        ...
                        if ($active) {
                            ...
                            $html .= "<a href='/forum/" .$this->idTopic. "/" .$i. "'
                                class='button-base'>
                                ";
                        }
                        $html .= $i;
